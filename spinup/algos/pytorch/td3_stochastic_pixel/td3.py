...
import time
import spinup.algos.pytorch.td3_stochastic_pixel.core as core
...
class CNNCritic(nn.Module):
    def __init__(self, act_dim, img_stack,
        ...
        full_layer_sizes = [int(256)+act_dim] + full_hidden_sizes + [1]
...
class CNNActor(nn.Module):
    def __init__(self, act_dim, act_limit, img_stack,
        ...
        for f_i in range(len(full_layer_sizes)-2):
            self.full_layers += [nn.Linear(full_layer_sizes[f_i], full_layer_sizes[f_i+1]), nn.ReLU()]
        self.mu_layer = nn.Linear(full_layer_sizes[-2], full_layer_sizes[-1])
        self.log_std_layer = nn.Linear(full_layer_sizes[-2], full_layer_sizes[-1])
        # self.alpha_head = nn.Sequential(nn.Linear(full_layer_sizes[-2], 3), nn.Softplus())
        # self.beta_head = nn.Sequential(nn.Linear(full_layer_sizes[-2], 3), nn.Softplus())
        self.apply(self._weights_init)
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        ...
        # return action, a_logp, alpha, beta, dist
        ...
        pi_action = torch.cat(((pi_action[:, 0]*2-1).reshape(-1,1), pi_action[:, 1:]), dim=1)
        return pi_action, logp_pi, mu, log_std, pi_distribution
...
class CNNActorCritic(nn.Module):
    ...
    def act(self, obs, deterministic=False):
        with torch.no_grad():
            a,_,_,_,_ = self.pi(obs, deterministic, False)
            import pdb;
            pdb.set_trace()
            return a.cpu().numpy()
...
def td3(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        steps_per_epoch=4000, epochs=100, replay_size=int(1e6), gamma=0.99, alpha = 0.2,
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            ...
            a2, logp_a2, mu_a2, log_std_a2, a2_dist = ac.pi(o2)
    ...
    def get_action(o, deterministic=False):
        return ac.act(torch.as_tensor(o.reshape((1,)+o.shape), dtype=torch.float32).to(device), deterministic)[0]
