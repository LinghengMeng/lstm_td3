...
import time
import spinup.algos.pytorch.td3_pixel_shareCNN.core as core
...
#
#######################################################################################
class CNNSharedFeatureExtractor(nn.Module):
    def __init__(self, img_stack):
        super(CNNSharedFeatureExtractor, self).__init__()
        # CNN layers
        ...
        self.cnn_layers += [nn.Conv2d(img_stack, 8, kernel_size=4, stride=2), # (8, 47, 47)
                            ...
                            nn.Sigmoid(),  # nn.ReLU(), # match the value range with action range.
                            nn.Flatten()]
...
class FCNCritic(nn.Module):
    def __init__(self, act_dim, ftr_dim, full_hidden_sizes=[256, 256]):
        super(FCNCritic, self).__init__()
        self.full_layers = nn.ModuleList()

        # Fully connection layers
        full_layer_sizes = [ftr_dim+act_dim*10] + full_hidden_sizes + [1]
        ...
        # self.act_full_layers = nn.ModuleList()
        # self.act_full_layers += [nn.Linear(act_dim, 256), nn.ReLU()]
        #
        # # Fully connection layers
        # full_layer_sizes = [ftr_dim+256] + full_hidden_sizes + [1]
        # for f_i in range(len(full_layer_sizes) - 2):
        #     self.full_layers += [nn.Linear(full_layer_sizes[f_i], full_layer_sizes[f_i + 1]), nn.ReLU()]
        # self.full_layers += [nn.Linear(full_layer_sizes[-2], full_layer_sizes[-1]), nn.Identity()]

    def forward(self, ftr, act):
        # for act_f_layer in self.act_full_layers:
        #     act = act_f_layer(act)
        # import pdb; pdb.set_trace()
        # Note: 1. the dimenions of the extracted feature and the actor are extremely unbalanced
        #       2. the value range of the extracted feature is [0, inf), while the value range of the action is [0,1]
        act = act.repeat(1, 10)
        x = torch.cat((ftr, act), dim=1)
        for f_layer in self.full_layers:
            x = f_layer(x)
        return torch.squeeze(x, -1)  # Critical to ensure q has right shape.

class FCNActor(nn.Module):
    def __init__(self, act_dim, act_limit, ftr_dim, full_hidden_sizes=[256, 256]):
        super(FCNActor, self).__init__()
        self.act_limit = act_limit

        self.full_layers = nn.ModuleList()

        # Fully connection layers
        full_layer_sizes = [ftr_dim] + full_hidden_sizes + [act_dim]
        for f_i in range(len(full_layer_sizes)-2):
            self.full_layers += [nn.Linear(full_layer_sizes[f_i], full_layer_sizes[f_i+1]), nn.ReLU()]
...
class CNNActorCritic(nn.Module):
    def __init__(self, obs_dim, act_dim, act_limit, img_stack=4):
        ...
        self.ftr_dim = 256
        self.ftr_extractor = CNNSharedFeatureExtractor(img_stack)
        self.q1 = FCNCritic(act_dim, self.ftr_dim)
        self.q2 = FCNCritic(act_dim, self.ftr_dim)
        self.pi = FCNActor(act_dim, act_limit, self.ftr_dim)

    def pred_q(self, obs, act):
        ftr = self.ftr_extractor(obs)
        q1 = self.q1(ftr, act)
        q2 = self.q2(ftr, act)
        return q1, q2
...
def td3(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    q_params = itertools.chain(ac.ftr_extractor.parameters(), ac.q1.parameters(), ac.q2.parameters())
    pi_params = itertools.chain(ac.ftr_extractor.parameters(), ac.pi.parameters())
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            ...
            # Target Q-values
            q1_pi_targ, q2_pi_targ = ac_targ.pred_q(o2, a2)
    ...
    def compute_loss_pi(data):
        o = data['obs']
        q1_pi, q2_pi = ac.pred_q(o, ac.pred_a(o))
        return -q1_pi.mean()
    ...
    def update(data, timer):
        ...
        for h_i in range(1):
            if h_i % 2 == 0:
                q_grad_log['Q1Layer{}WeightGrad'.format(h_i)] = ac.ftr_extractor.cnn_layers[h_i].weight.grad.cpu().detach().numpy()
                q_grad_log['Q1Layer{}BiasGrad'.format(h_i)] = ac.ftr_extractor.cnn_layers[h_i].bias.grad.cpu().detach().numpy()
        for h_i in range(4):
            if h_i % 2 == 0:
                q_grad_log['Q1FCNLayer{}WeightGrad'.format(h_i)] = ac.q1.full_layers[h_i].weight.grad.cpu().detach().numpy()
                q_grad_log['Q1FCNLayer{}BiasGrad'.format(h_i)] = ac.q1.full_layers[h_i].bias.grad.cpu().detach().numpy()

                # # Random set value for grad equal to 0
                # ac.q1.full_layers[h_i].weight.grad[ac.q1.full_layers[h_i].weight.grad == 0] = np.random.uniform(-1,
                #                                                                                                 1) * 1e-4  # np.random.rand() * 1e-8
                # ac.q1.full_layers[h_i].bias.grad[ac.q1.full_layers[h_i].bias.grad == 0] = np.random.uniform(-1,
                #                                                                                             1) * 1e-4  # np.random.rand() * 1e-8
                #
                # q_grad_log['Q1FCNAfterTuneLayer{}WeightGrad'.format(h_i)] = ac.q1.full_layers[
                #     h_i].weight.grad.cpu().detach().numpy()
                # q_grad_log['Q1FCNAfterTuneLayer{}BiasGrad'.format(h_i)] = ac.q1.full_layers[h_i].bias.grad.cpu().detach().numpy()
        ...
        if timer % policy_delay == 0:
            ...
            for h_i in range(1):
                if h_i % 2 == 0:
                    grad_log['PiLayer{}WeightGrad'.format(h_i)] = ac.ftr_extractor.cnn_layers[h_i].weight.grad.cpu().detach().numpy()
                    grad_log['PiLayer{}BiasGrad'.format(h_i)] = ac.ftr_extractor.cnn_layers[h_i].bias.grad.cpu().detach().numpy()
            ...
            for h_i in range(4):
                if h_i % 2 == 0:
                    grad_log['PiFCNLayer{}WeightGrad'.format(h_i)] = ac.pi.full_layers[h_i].weight.grad.cpu().detach().numpy()
                    grad_log['PiFCNLayer{}BiasGrad'.format(h_i)] = ac.pi.full_layers[h_i].bias.grad.cpu().detach().numpy()

                    # Random set value for grad equal to 0
                    ac.pi.full_layers[h_i].weight.grad[ac.pi.full_layers[h_i].weight.grad == 0] = np.random.uniform(-1, 1)*1e-4 # np.random.rand() * 1e-8
                    ac.pi.full_layers[h_i].bias.grad[ac.pi.full_layers[h_i].bias.grad == 0] = np.random.uniform(-1, 1)*1e-4 # np.random.rand() * 1e-8
                    ...
                    #                                                                                      1) * grad_threshold
                    #
                    grad_log['PiFCNAfterTunedLayer{}WeightGrad'.format(h_i)] = ac.pi.full_layers[
                        h_i].weight.grad.cpu().detach().numpy()
                    grad_log['PiFCNAfterTunedLayer{}BiasGrad'.format(h_i)] = ac.pi.full_layers[
    ...
    def get_action(o, noise_scale):
        ...
        # a = np.asarray([np.clip(a[0], -1, 1)]+list(np.clip(a[1:], 0, 1)))
        #
        # import pdb; pdb.set_trace()
        return np.asarray([np.clip(a[0], -1, 1)]+list(np.clip(a[1:], 0, 1)))
    ...
    def test_agent(num_test_episodes):
        ...
        for j in range(num_test_episodes):
            o, d, die, ep_ret, ep_len = test_env.reset(), False, 0, 0, 0
            while not(d  or die or (ep_len == max_ep_len)):
                # Take deterministic actions at test time (noise_scale=0)
                o, r, d, die = test_env.step(get_action(o, 0))
                ep_ret += r
                ep_len += 1
    ...
    for t in range(total_steps):
        ...
        if d or die or (ep_len == max_ep_len):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            for h_i in range(4):
                if h_i % 2 == 0:
                    logger.log_tabular('Q1FCNLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
                    logger.log_tabular('Q1FCNLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
            # for h_i in range(4):
            #     if h_i % 2 == 0:
            #         logger.log_tabular('Q1FCNAfterTuneLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
            #         logger.log_tabular('Q1FCNAfterTuneLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
            #
            # # for h_i in range(len(ac.pi.cnn_layers)):
            for h_i in range(1):
                if h_i % 2 == 0:
                    logger.log_tabular('PiLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
                    logger.log_tabular('PiLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
            # # for h_i in range(len(ac.pi.cnn_layers)):
            # for h_i in range(1):
            #     if h_i % 2 == 0:
            ...
            for h_i in range(4):
                if h_i % 2 == 0:
                    logger.log_tabular('PiFCNLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
                    logger.log_tabular('PiFCNLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
            for h_i in range(4):
                if h_i % 2 == 0:
                    logger.log_tabular('PiFCNAfterTunedLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
                    logger.log_tabular('PiFCNAfterTunedLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
