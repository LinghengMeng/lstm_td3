...
import time
import spinup.algos.pytorch.ddpg_double_distribution.core as core
...
class MLPActorCritic(nn.Module):
    def __init__(self, obs_dim, act_dim, act_limit, critic_hidden_sizes=[256, 256], actor_hidden_sizes=[256, 256], dist_type='Normal'):
        ...
        self.q1 = MLPCritic(obs_dim, act_dim, critic_hidden_sizes, dist_type=dist_type)
        self.q2 = MLPCritic(obs_dim, act_dim, critic_hidden_sizes, dist_type=dist_type)
...
def ddpg(env_name, partially_observable=False,
    ...
    def compute_loss_q(data):
        ...
        q1_mu, q1_log_std, q1_distribution, q1_hid_activation = ac.q1(o, a)
        q2_mu, q2_log_std, q2_distribution, q2_hid_activation = ac.q2(o, a)
        q1 = q1_distribution.rsample()
        q2 = q2_distribution.rsample()

        # Bellman backup for Q function
        with torch.no_grad():
            pi_targ, _ = ac_targ.pi(o2)
            q1_pi_mu_targ, q1_pi_log_std_targ, q1_pi_distribution_targ, _ = ac_targ.q1(o2, pi_targ)
            q2_pi_mu_targ, q2_pi_log_std_targ, q2_pi_distribution_targ, _ = ac_targ.q2(o2, pi_targ)

            # q1_pi_distribution_targ_entropy = q1_pi_distribution_targ.entropy()
            # q1_pi_distribution_targ_entropy = q1_pi_distribution_targ.entropy()

            q1_pi_targ = q1_pi_distribution_targ.rsample()
            q2_pi_targ = q2_pi_distribution_targ.rsample()
        ...
        loss_info = dict(Q1MuVals=q1_mu.detach().cpu().numpy(),
                         Q1LogStdVals=q1_log_std.detach().cpu().numpy(),
                         Q2MuVals=q2_mu.detach().cpu().numpy(),
                         Q2LogStdVals=q2_log_std.detach().cpu().numpy())
    ...
    def compute_loss_pi(data):
        ...
        q1_pi_mu, q1_pi_log_std, q1_pi_distribution, _ = ac.q1(o_, a_)
        q2_pi_mu, q2_pi_log_std, q2_pi_distribution, _ = ac.q2(o_, a_)
        q1_pi = q1_pi_distribution.rsample()
        q2_pi = q2_pi_distribution.rsample()
        q_pi = torch.min(q1_pi, q2_pi)
        #
        loss_pi = - q_pi.mean()
        # loss_pi = - q1_pi.mean() - q1_pi_distribution.entropy().mean()
        # loss_pi = - q1_pi.mean() - q2_pi.mean()
        # loss_pi = - q1_pi.mean() - q1_pi_distribution.entropy().mean()
        # loss_pi = - q1_pi_mu.mean()# - q1_pi_log_std.mean()
        return loss_pi
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            logger.log_tabular('Q1MuVals', with_min_and_max=True)
            logger.log_tabular('Q1LogStdVals', with_min_and_max=True)
            logger.log_tabular('Q2MuVals', with_min_and_max=True)
            logger.log_tabular('Q2LogStdVals', with_min_and_max=True)
