...
import time
import spinup.algos.pytorch.ddpg_prudent_policy.core as core
...
class ReplayBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size):
        ...
        self.q_pi_buf = {i: [] for i in range(size)}
    ...
    def add_sample_hist(self, sample_idxs, q_pi, t):
        """Add prediction history."""
        for i in range(len(sample_idxs)):
            self.q_pi_buf[sample_idxs[i]].append(q_pi[i])
    ...
    def sample_batch(self, batch_size=32, device=None, sample_type='genuine_random'):
        ...
        batch_hist = dict(q_pi_hist=[self.q_pi_buf[i] for i in idxs],
...
def ddpg(env_name, partially_observable=False,
    ...
    def compute_loss_pi(data, batch_hist):
        o = data['obs']
        q_pi = ac.q(o, ac.pi(o))

        old_q = q_pi
        for i in range(len(batch_hist['q_pi_hist'])):
            if len(batch_hist['q_pi_hist'][i])!=0:
                old_q[i] = batch_hist['q_pi_hist'][i][-1].astype('float64')

        with torch.no_grad():
            epsilon = 1e-8
            q_scale = (old_q + torch.sign(q_pi-old_q)*epsilon)/(q_pi+1e-8)

        q_pi = q_scale*q_pi
        return -q_pi.mean(), q_pi
    ...
    def update(data, batch_hist, t):
        ...
        loss_pi, q_pi = compute_loss_pi(data, batch_hist)
        loss_pi.backward()
        pi_optimizer.step()
    ...
    for t in range(total_steps):
        ...
        if t >= update_after and t % update_every == 0:
            for _ in range(update_every):
                ...
                q_pi = update(data=batch, batch_hist=batch_hist, t=t)
                replay_buffer.add_sample_hist(batch_idxs, q_pi, t)
