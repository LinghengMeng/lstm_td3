...
import time
import spinup.algos.pytorch.ddpg_orig.core as core
...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    ac = MLPActorCritic(obs_dim, act_dim, act_limit,
                        ...
                        actor_hidden_sizes=ac_kwargs['act_hidden_sizes'])
    ac_targ = deepcopy(ac)
    ...
    def update(data):
        ...
        # Log q grad
        q_grad_log = {}
        for h_i in range(len(ac.q.layers)):
            if h_i % 2 == 0:
                q_grad_log['QLayer{}WeightGrad'.format(h_i)] = ac.q.layers[h_i].weight.grad.cpu().detach().numpy()
                q_grad_log['QLayer{}BiasGrad'.format(h_i)] = ac.q.layers[h_i].bias.grad.cpu().detach().numpy()
        # Clip gradient
        # import pdb; pdb.set_trace()
        nn.utils.clip_grad_value_(ac.q.parameters(), 0.1)
        for h_i in range(len(ac.q.layers)):
            if h_i % 2 == 0:
                q_grad_log['ClippedQLayer{}WeightGrad'.format(h_i)] = ac.q.layers[h_i].weight.grad.cpu().detach().numpy()
                q_grad_log['ClippedQLayer{}BiasGrad'.format(h_i)] = ac.q.layers[h_i].bias.grad.cpu().detach().numpy()
        ...
        for h_i in range(len(ac.pi.layers)):
            if h_i % 2 == 0:
                pi_grad_log['PiLayer{}WeightGrad'.format(h_i)] = ac.pi.layers[h_i].weight.grad.cpu().detach().numpy()
                pi_grad_log['PiLayer{}BiasGrad'.format(h_i)] = ac.pi.layers[h_i].bias.grad.cpu().detach().numpy()

                # # Random set value for grad equal to 0
                # ac.pi.layers[h_i].weight.grad[ac.pi.layers[h_i].weight.grad == 0] = np.random.rand() * 1e-6
                # ac.pi.layers[h_i].bias.grad[ac.pi.layers[h_i].bias.grad == 0] = np.random.rand() * 1e-6

                # grad_log['AfterTunedLayer{}WeightGrad'.format(h_i)] = ac.pi.layers[
                #     h_i].weight.grad.cpu().detach().numpy()
                # grad_log['AfterTunedLayer{}BiasGrad'.format(h_i)] = ac.pi.layers[h_i].bias.grad.cpu().detach().numpy()

        pi_optimizer.step()
        logger.store(LossPi=loss_pi.item(), **pi_grad_log)
    ...
    for t in range(total_steps):
        ...
        if (t+1) % steps_per_epoch == 0:
            ...
            for h_i in range(len(ac.q.layers)):
                if h_i % 2 == 0:
                    logger.log_tabular('QLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
                    logger.log_tabular('QLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
            for h_i in range(len(ac.q.layers)):
                if h_i % 2 == 0:
                    logger.log_tabular('ClippedQLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
                    logger.log_tabular('ClippedQLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
            ...
            # for h_i in range(len(ac.pi.layers)):
            #     if h_i % 2 == 0:
            #         logger.log_tabular('AfterTunedLayer{}WeightGrad'.format(h_i), with_min_and_max=True)
            #         logger.log_tabular('AfterTunedLayer{}BiasGrad'.format(h_i), with_min_and_max=True)
