...
def ddpg(env_name, partially_observable=False,
    ...
    def compute_loss_pi(data):
        ...
        a_avg_hid_activation = torch.cat(a_hid_activation, dim=1).mean(axis=0)
        a_rho = torch.ones(a_avg_hid_activation.shape).to(DEVICE) * actor_sparsity_parameter_rho
        a_sparsity_penalty = torch.sum(
            a_rho * torch.log(a_rho / a_avg_hid_activation) + (1 - a_rho) * torch.log(
                (1 - a_rho) / (1 - a_avg_hid_activation)))
