...
import time
import spinup.algos.pytorch.ddpg_distribution_approx.core as core
...
def ddpg(env_name, partially_observable=False,
         ...
         flicker_prob=0.2, random_noise_sigma=0.1, random_sensor_missing_prob=0.1, gamma=0.99,
         dist_type = 'Normal',
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            ...
            backup = backup_distribution.rsample()

            q_mu, q_log_std, q_distribution, q_hid_activation = ac.q(o, a)
            q_pred = q_distribution.rsample()

        clip_param = 0.1
        ratio = torch.exp(q_distribution.log_prob(q) - backup_distribution.log_prob(backup))
        calipped_ratio = torch.clamp(ratio, 1.0 - clip_param, 1.0 + clip_param)
        # import pdb;
        # pdb.set_trace()
        # loss_q = ((torch.min(ratio, calipped_ratio)*(q - backup))**2).mean()
        loss_q_1 = ((torch.min(ratio, calipped_ratio) * torch.abs((q_pred - backup)))).mean()
        loss_q_2 = ((q - backup)** 2).mean()
        ...
        # lagrange_multiplier = 1e-2
        # loss_q = q_mse + lagrange_multiplier * kl_loss
        # import pdb; pdb.set_trace()
        ...
        loss_info = dict(QMuVals=q_mu.detach().cpu().numpy(),
                         ...
                         LossQ1=loss_q_1.cpu().detach().numpy(), LossQ2=loss_q_2.cpu().detach().numpy())
    ...
    def compute_loss_pi(data):
        ...
        q_pi = q_pi_distribution.rsample()
